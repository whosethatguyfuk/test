...
// Uses OS entropy (getrandom on Linux, /dev/urandom fallback)
unsigned long long int makeSeed() {
    unsigned long long int seed = 0;
    unsigned char *pseed = (unsigned char *)&seed;
#ifdef __linux__
    if (getrandom(pseed, sizeof(seed), 0) == (ssize_t)sizeof(seed))
        return seed;
#endif
    FILE *f = fopen("/dev/urandom", "rb");
    if (f) {
        if (fread(pseed, 1, sizeof(seed), f) == sizeof(seed)) {
            fclose(f);
            return seed;
        }
        fclose(f);
...
void __global__ vanity_scan(curandState* state, int* keys_found, int* gpu, int* exec_count) {
	...
	for (int attempts = 0; attempts < ATTEMPTS_PER_EXECUTION; ++attempts) {
		// Fresh random bytes per key - curand seeded from OS entropy (secure)
		for (int i = 0; i < 32; ++i) {
			float r = curand_uniform(&localState);
			seed[i] = (uint8_t)(r * 256.0f);
		}
		// sha512_init Inlined
		md.curlen   = 0;
		md.length   = 0;
		...
		// this.
				...
				if ( !(prefixes[i][j] == '?') && !(prefixes[i][j] == key[j]) ) {
					...
					break;
                        ...
                        bool suffix_match = (keylen >= suffix_letter_counts[i]);
                        if (suffix_match) {
                                for (int j = 0; j < suffix_letter_counts[i]; ++j) {
					if ( !(suffixes[i][j] == '?') && !(suffixes[i][j] == key[keylen - suffix_letter_counts[i] + j]) ) {
						suffix_match = false;
						break;
					}
                                }
		...
		// (Seed is refreshed from curand at top of loop - secure)
	}
